...
public class CarController : NetworkBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // Only the driver gets input
        if (AmITheDriver() == true)
        {
            GetInput();
        }

        // Only the owner handles physics (host)
        if (IsOwner)
        {
            HandleMotor();
            HandleSteering();
    ...
    // and handle car entering/exiting logic
    private void Update()
    {
        var netObj = player?.GetComponent<NetworkObject>();
        if (netObj == null || !netObj.IsLocalPlayer) return;
    ...
    // Handle car entering logic
    // This is called when the player presses E while colliding with the car
    // // It will try to claim the driver seat first, needs to request to the server because he does not manage the seat occupants list
    ...
    // Make it stick transforms to the car
    ...
    // +++ CAR INPUT NETOWORK LOGIC +++
    // THE CLIENT DOES NOT HANDLE PHYSICS NOR INPUTS, SO HE NEEDS TO SEND TO THE HOST (MIGHT HAVE SOME TRANSOFM MISPOSITONS)
    ...
    // +++ END CAR INPUT NETWORK LOGIC +++

    // +++ CAR HANDLING LOGIC +++ (LOCAL)
