...
public class MeshGeneratorV2 : MonoBehaviour
{
    ...
    private void SpawnPlayer()
    {
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                ...
                if (noiseHeight > highestPoint)
                {
                highestPoint = noiseHeight;
                spawnPosition = new Vector3(mesh.vertices[i].x * MESH_SCALE, noiseHeight * 1, mesh.vertices[i].z * MESH_SCALE);
                }
            }

            Instantiate(Player, spawnPosition, Quaternion.identity);
            Instantiate(Camera, spawnPosition, Quaternion.identity);
            playerSpawned = true;
    }
